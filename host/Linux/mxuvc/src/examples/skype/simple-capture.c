...
int main(int argc, char **argv)
{
	...
	if(strncmp(VIDEO_BACKEND, "libusb-uvc", 10) == 0) {
		ret = mxuvc_video_init("libusb-uvc",
				"vid = " VID "; pid = " PID ";");
	} else if (strncmp(VIDEO_BACKEND, "v4l2", 4) == 0) {
		ret = mxuvc_video_init("v4l2","dev_offset=0");
	} else {
		printf("No compatible video backend found");
		return 1;
	}
	if(ret < 0)
		return 1;
	...
	/* Set audio settings*/
	ret = mxuvc_audio_set_format(AUD_CH1, AUD_FORMAT_PCM_RAW);
	if(ret < 0)
		goto error;
