...
int main(int argc, char **argv)
{
	...
	ret = mxuvc_burnin_init(font_size, file);	
	if(ret < 0)
	  return 1;
	printf("burninit done\n");
	...
	while(counter--) {
		...
		if(counter == 18){
		ret = mxuvc_burnin_add_text( 0, "text0", sizeof("text0"), 0, 0 );
			if(ret < 0)
				goto error;
			ret = mxuvc_burnin_add_text( 1, "text1", sizeof("text1"), 0, 1200 );
			if(ret < 0)
				goto error;
			ret = mxuvc_burnin_add_text( 2, "text2", sizeof("text2"), 1200, 0 );
			if(ret < 0)
				goto error;
		}
		if(counter == 18){
			ret = mxuvc_burnin_show_time(1200, 1200, 1);
			if(ret < 0)
				goto error;
			ret = mxuvc_burnin_update_time(10, 30, 40);
			if(ret < 0)
				goto error;
		}

		if(counter == 5){
			ret = mxuvc_burnin_remove_text( 1);
			if(ret < 0)
				goto error;
			ret = mxuvc_burnin_hide_time();
			if(ret < 0)
				goto error;
		}
		fflush(stdout);
