...
void aalarm_callback(unsigned int audioIntensityDB, void *data)
{
	printf("audio alarm Intensity %ddb\n",audioIntensityDB);
}

void malarm_callback(alert_motion_data *malarm, void *data)
{
	printf("motion alarm reg_id %d transition %d mbs_in_motion %d\
		avg_motion %d PTS_high %d PTS_low %d\n",\
				malarm->reg_id, malarm->transition, malarm->mbs_in_motion,\
				malarm->avg_motion, malarm->PTS_high, malarm->PTS_low);
}
