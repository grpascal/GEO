...
int decode_jpeg_raw (const unsigned char *jpeg_data, int len,
                     int itype, int ctype, struct framebuffer *frm)
{
   int numfields, hsf[3], vsf[3], field, yl, yc, x, y = 0, i, xsl, xsc, xs, xd,
       hdown;
   unsigned char *raw0, *raw1, *raw2;
   int width = frm->width;
   ...
   jpeg_buffer_src (&dinfo, (unsigned char*)jpeg_data, len);
   ...
   if (ctype ==  Y4M_CHROMA_420JPEG) {
	frm->size = 3*height*width/2;
   	frm->data = malloc(frm->size);
   	raw0 = (unsigned char*) frm->data;
	raw1 = raw0 + height*width;
	raw2 = raw1 + height*width/4;
   } else {/* Y4M_CHROMA_422 */
	frm->size = 2*height*width;
   	frm->data = malloc(frm->size);
   	raw0 = (unsigned char*) frm->data;
	raw1 = raw0 + height*width;
	raw2 = raw1 + height*width/2;
  }
